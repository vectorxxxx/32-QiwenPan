...
public class EditCallback implements Callback
{
    ...
    public int handle(Track body, String fileName) {  // handle the callback when the document is being edited
        ...
        Action action = body
                .getActions()
                .get(0);  // get the user ID who is editing the document
        if (action
                .getType()
                .equals(com.qiwenshare.file.office.documentserver.models.enums.Action.edit)) {  // if this value is not equal to the user ID
            String user = action.getUserid();  // get user ID
            if (!body
                    .getUsers()
                    .contains(user)) {  // if this user is not specified in the body
