...
public class DefaultCallbackManager implements CallbackManager
{
    ...
    public void processSave(Track body, String fileName) {  // file saving process
        ...
        if (lastVersion
                .toFile()
                .exists()) {  // if the last file version exists
            ...
            String versionDir = documentManager.versionDir(histDir
                            .toAbsolutePath()
                            .toString(),  // get the file version directory
                    storagePathBuilder.getFileVersion(histDir
                            .toAbsolutePath()
                            .toString(), false), true);
            ...
            storageMutator.moveFile(lastVersion,
                    Paths.get(versionDir + File.separator + "prev" + curExt));  // move the last file version to the file version directory with the "prev" postfix
            ...
            jsonChanges.put("changes", body
                    .getHistory()
                    .getChanges());  // put the changes to the json object
            jsonChanges.put("serverVersion", body
                    .getHistory()
    ...
    public void commandRequest(String method, String key, HashMap meta) {  // create a command request
        ...
        if (jwtManager.tokenEnabled())  // check if a secret key to generate token exists or not
        {
            ...
            connection.setRequestProperty(documentJwtHeader.equals("") ?
                                          "Authorization" :
                                          documentJwtHeader, "Bearer " + headerToken);  // add a header Authorization with a header token and Authorization prefix in it
    ...
    public void processForceSave(Track body, String fileName) {  // file force saving process
        ...
        if (isSubmitForm) {  // if the form is submitted
            if (newFileName) {
                fileName = documentManager.getCorrectName(
                        fileUtility.getFileNameWithoutExtension(fileName) + "-form" + downloadExt);  // get the correct file name if it already exists
            }
            else {
